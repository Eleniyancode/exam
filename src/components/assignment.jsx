...
//   console.log(result);
  ...
  const assignmentResult = await octokit.request('GET /repos/Eleniyancode/assignment', {
    owner: 'Eleniyancode',
    repo: 'assignment',
    headers: {
      'X-GitHub-Api-Version': '2022-11-28'
  ...
  return (
    ...
    <Box bg="blue" w="300px" h="100px" margin="auto" mt="20px" display="flex" flexDirection="column" justifyContent="center">
      <h1 style={{  color: "white", fontSize: "20px", textAlign: "center", paddingTop: "20px" }}>
          ...
          <Link to="src/repospages/assignment.jsx" style={{textAlign:"center", color: "white", fontSize: "20px", backgroundColor: "yellow"}} >{assignmentData.name}</Link>
      </Box>
      </Box></>
  );
};
