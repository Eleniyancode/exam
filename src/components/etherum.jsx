...
//   console.log(result);
  ...
  const ethereumResult = await octokit.request('GET /repos/Eleniyancode/ethereum', {
    owner: 'Eleniyancode',
    repo: 'ethereum',
    headers: {
      'X-GitHub-Api-Version': '2022-11-28'
  ...
  const ethereumData = ethereumResult.data
  ...
  return (
    ...
    <Box bg="blue" w="300px" h="100px" margin="auto" mt="20px" display="flex" flexDirection="column" justifyContent="center">
      <h1 style={{  color: "white", fontSize: "20px", textAlign: "center", paddingTop: "20px" }}>
          ...
          <Link to="src/repospages/etherum.jsx" style={{textAlign:"center", color: "white", fontSize: "20px", backgroundColor: "yellow"}} >{ethereumData.name}</Link>
      </Box>
      </Box></>
  );
};
